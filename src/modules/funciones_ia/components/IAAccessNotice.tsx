...
export function getIAAccessMessage(response: UsarFuncionIAResponse | null) {
  if (!response) return '';
  return FALLBACK_MESSAGES[response.codigo] || response.mensaje || 'No se pudo validar el acceso a la función IA.';
}

interface IAAccessNoticeProps {
  response: UsarFuncionIAResponse | null;
  onClose?: () => void;
  variant?: 'modal' | 'inline';
}

export default function IAAccessNotice({ response, onClose, variant = 'modal' }: IAAccessNoticeProps) {
  ...
  const content = (
    <div className="w-full max-w-lg rounded-xl border p-5 shadow-xl space-y-4 animate-in fade-in zoom-in-95 duration-200" style={{ borderColor: 'var(--line)', backgroundColor: 'var(--panel)' }}>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-end gap-2">
        ...
        {onClose ? (
          <button type="button" className="px-4 py-2 rounded-lg border hover:bg-[var(--panel-soft)] transition" style={{ borderColor: 'var(--line)' }} onClick={onClose}>
            Cerrar
          </button>
        ) : null}
      </div>
